          ...
          <div style={{background:C.surface,borderRadius:22,padding:'18px',marginBottom:12,boxShadow:`0 2px 14px rgba(0,0,0,${dark?.15:.07})`}}>
            ...
            {supabaseUser&&(
              <button className="tap" onClick={async()=>{
                ...
                const localPerfil = LS.get('perfil',null);
                const localObj    = LS.get('obj','mantener');
                setToast(`🔍 Intentando sync... nombre="${localNombre}"`);
                const ok = await syncProfile(supabaseUser.id,{nombre:localNombre,perfil:localPerfil,obj:localObj});
                if(ok){
                  setToast(`✅ syncProfile OK — nombre="${localNombre}"`);
                } else {
                  setToast(`❌ syncProfile FALLÓ — revisa consola`);
                }
              }} style={{marginTop:8,width:'100%',padding:'12px',borderRadius:14,border:`1px solid #007AFF`,background:'#007AFF18',color:'#007AFF',fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:F}}>
                🔧 Diagnóstico: forzar sync
              </button>
            )}
