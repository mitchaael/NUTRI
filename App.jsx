...
/* ═══════════════════════════════════════════════════════
   CONSTANTS
═══════════════════════════════════════════════════════ */
const CATS = ["Todas","Lácteos","Carnes","Cecinas","Panes","Cereales","Snacks","Bebidas","Frutas","Verduras","Legumbres","Granos","Pescados","Huevos","Comidas CL","Congelados","Comida rápida","Preparados","Aceites","Condimentos","Suplementos"];
  ...
  const steps=[
    ...
    <div key={0} style={{display:'flex',flexDirection:'column',alignItems:'center',padding:'40px 28px',gap:24,textAlign:'center'}}>
      ...
      <button onClick={()=>nombre.trim()&&setStep(1)} style={{
        ...
        background:nombre.trim()?C.text:'#CCC',color:nombre.trim()?(dark?C.bg:'white'):'white',
    ...
    <div style={{fontFamily:F,minHeight:'100vh',background:C.bg,paddingBottom:88,transition:'background .3s'}}>
      ...
      <div style={{padding:'14px 14px 0'}}>
        ...
        {tab===1&&<div className={tabAnim}>
          ...
          <div style={{display:'flex',gap:7,overflowX:'auto',paddingBottom:4,marginBottom:14,scrollbarWidth:'none'}}>
            {MEALS.map(m=>(
              <button key={m} className="tap" onClick={()=>setMeal(m)} style={{
                ...
                background:meal===m?MC[m]:C.surfaceAlt,color:meal===m?'white':C.textSec,
          ...
          <div style={{display:'flex',gap:6,overflowX:'auto',paddingBottom:6,marginBottom:10,scrollbarWidth:'none'}}>
            {CATS.map(c=>(
              <button key={c} className="tap" onClick={()=>setCat(c)} style={{flexShrink:0,padding:'5px 12px',borderRadius:12,border:cat===c?'none':`1px solid ${C.border}`,background:cat===c?C.primary:C.surface,color:cat===c?'white':C.textSec,fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:F,transition:'all .15s'}}>{c}</button>
            ))}
          </div>
      ...
      {toast&&(
        <div style={{
          ...
          background:C.primary,color:dark?C.bg:'#FFFFFF',
        ...
        {[{icon:'🏠',lbl:'Inicio'},{icon:'➕',lbl:'Agregar'},{icon:'📋',lbl:'Mi Día'},{icon:'🎯',lbl:'Objetivos'}].map(({icon,lbl},i)=>(
          <button key={i} className="nav-btn tap" onClick={()=>goTab(i)} style={{
            ...
            <div style={{
              ...
              background:tab===i?`${C.accent}18`:'transparent',
