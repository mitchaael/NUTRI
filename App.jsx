...
function AuthScreen({ onAuth }) {
  ...
  const C = LIGHT; const F = "'SF Pro Display','Helvetica Neue',sans-serif";
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'#F2F2F7', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'24px', fontFamily:F }}>
      ...
      <div style={{ marginBottom:32, textAlign:'center' }}>
        ...
        <div style={{ fontSize:28, fontWeight:800, color:'#1C1C1E', letterSpacing:'-1px' }}>Calorú</div>
        <div style={{ fontSize:13, color:'#6D6D72', marginTop:4 }}>Tu app de nutrición chilena</div>
      </div>

      {/* Card */}
      <div style={{ width:'100%', maxWidth:380, background:'white', borderRadius:24, padding:'24px', boxShadow:'0 4px 24px rgba(0,0,0,0.08)' }}>
        {/* Tabs */}
        ...
        <div style={{ marginBottom:12 }}>
          <input
            ...
            style={{ width:'100%', padding:'14px', borderRadius:14, border:'1.5px solid #E5E5EA', fontSize:15, fontFamily:F, outline:'none', boxSizing:'border-box', color:'#1C1C1E' }}
          />
        </div>
        <div style={{ marginBottom:16 }}>
          <input
            type="password" placeholder="Contraseña (mín. 6 caracteres)" value={pass}
        ...
        {msg   && <div style={{ fontSize:12, color:'#34C759', marginBottom:10, textAlign:'center', fontWeight:600 }}>{msg}</div>}

        <button
          onClick={handle} disabled={loading}
          style={{ width:'100%', padding:'15px', borderRadius:16, border:'none', background: loading ? '#C7C7CC' : '#1C1C1E', color:'white', fontSize:15, fontWeight:800, cursor: loading ? 'default' : 'pointer', fontFamily:F, transition:'background .2s' }}
        >
        ...
        <button
          onClick={()=>onAuth(null)}
          style={{ width:'100%', padding:'12px', marginTop:10, borderRadius:16, border:'1px solid #E5E5EA', background:'transparent', color:'#6D6D72', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:F }}
        >
          Continuar sin cuenta
        </button>
      </div>
      <div style={{ fontSize:11, color:'#C7C7CC', marginTop:16, textAlign:'center' }}>
        Tus datos se guardan de forma segura con cifrado
      </div>
    </div>
  );
}
...
@keyframes fadeUp{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}}
  ...
  /* ── Restore desde Supabase al hacer login ── */
  useEffect(()=>{
    if(!supabaseUser) return;
    (async()=>{
      setSyncStatus('syncing');
      try {
        // ── Primera vez con esta cuenta: subir datos locales a Supabase ──
        const pushKey = 'caloru_pushed_' + supabaseUser.id;
        if(!LS.get(pushKey, false) && (nombre || log.length || weightHistory.length)) {
          if(nombre) await syncProfile(supabaseUser.id, {nombre, perfil, obj});
          ...
          // Subir últimos 30 días
          for(let i=0; i<30; i++){
            const d=new Date(); d.setDate(d.getDate()-i);
            const key=d.toISOString().slice(0,10);
            const dayLog=LS.get('log_'+key,[]);
            const dayAgua=LS.get('agua_'+key,0);
            const dayEx=LS.get('ex_'+key,[]);
            if(dayLog.length||dayAgua||dayEx.length)
              await syncDay(supabaseUser.id, key, dayLog, dayAgua, dayEx);
          }
          if(weightHistory.length) await syncWeight(supabaseUser.id, weightHistory);
          LS.set(pushKey, true);
          setSyncStatus('done');
          setTimeout(()=>setSyncStatus('idle'),2000);
          return; // datos ya están en local, no restaurar
        }

        // ── Volvió a entrar: restaurar solo si local está vacío ──
        const hasLocalData = nombre || log.length || LS.get('log_'+todayKey(),[]).length;
        if(hasLocalData){ setSyncStatus('idle'); return; }

        const data = await restoreFromSupabase(supabaseUser.id);
        if(data.profile?.nombre){
          setNombre(data.profile.nombre); LS.set('nombre',data.profile.nombre);
          if(data.profile.perfil){ setPerfil(data.profile.perfil); LS.set('perfil',data.profile.perfil); }
          if(data.profile.obj){ setObj(data.profile.obj); LS.set('obj',data.profile.obj); }
          LS.set('onboarded',true); setOnboarded(true);
        }
        if(data.settings){
          if(data.settings.allergens?.length){ setUserAllergens(data.settings.allergens); LS.set('allergens',data.settings.allergens); }
          if(data.settings.vegan_mode!=null){ setVeganMode(data.settings.vegan_mode); LS.set('veganMode',data.settings.vegan_mode); }
        }
        if(data.logs?.length){
          data.logs.forEach(row=>{
            if(!LS.get('log_'+row.date,[]).length && row.log?.length) LS.set('log_'+row.date, row.log);
            if(!LS.get('agua_'+row.date,0) && row.agua>0) LS.set('agua_'+row.date, row.agua);
            if(!LS.get('ex_'+row.date,[]).length && row.exercises?.length) LS.set('ex_'+row.date, row.exercises);
          });
          const todayRow=data.logs.find(r=>r.date===todayKey());
          if(todayRow && !log.length && todayRow.log?.length){
            setLog(todayRow.log);
            if(todayRow.agua) setAgua(todayRow.agua);
            if(todayRow.exercises?.length) setExercises(todayRow.exercises);
          }
        }
        if(data.weights?.length && !weightHistory.length){
          const wh=data.weights.map(r=>({date:r.date,label:r.date.slice(5),w:r.weight}));
