  ...
  const handle = async () => {
    ...
    try {
      if (mode === 'register') {
        ...
        if (e) throw e;
        // Registro exitoso — la sesión se crea automáticamente
        setMsg('');
      } else {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      try {
        ...
        // ── Dispositivo nuevo: bloquear onboarding hasta que Supabase responda ──
        const localNombre    = LS.get('nombre','');
        const localLog       = LS.get('log_'+todayKey(),[]);
        const localOnboarded = LS.get('onboarded', false);
        const hasRealLocalData = localOnboarded && localNombre.length > 0;
        if(hasRealLocalData){ setSyncStatus('idle'); return; }

        // Bloquear pantalla de onboarding mientras cargamos
        ...
        if(data.logs?.length){
          // Escribir todos los días en localStorage
          ...
          // Cargar el día de hoy en estado React
        ...
        if(data.weights?.length){
          const wh = data.weights.map(r=>({date:r.date, label:r.date.slice(5), w:parseFloat(r.weight)}));
          setWeightHistory(wh);  LS.set('weightHistory', wh);
        }
        setSyncStatus('done');
        setTimeout(()=>setSyncStatus('idle'), 2000);
      } catch(e){ setSyncStatus('error'); }
      finally { setRestoringData(false); }
    })();
  },[supabaseUser?.id]);
