          ...
          <div style={{background:C.surface,borderRadius:22,padding:'18px',marginBottom:12,boxShadow:`0 2px 14px rgba(0,0,0,${dark?.15:.07})`}}>
            ...
            {supabaseUser&&(
              <button className="tap" onClick={async()=>{
                ...
                const localPerfil = LS.get('perfil',null);
                const localObj    = LS.get('obj','mantener');
                setToast(`🔍 uid=${supabaseUser.id.slice(0,8)}... nombre="${localNombre}"`);
                // Llamada directa para ver el error real
                ...
                setTimeout(()=>{
                  if(error){
                    setToast(`❌ ERROR: ${error.message} (${error.code})`);
                  } else {
                    setToast(`✅ OK! nombre="${localNombre}" guardado`);
                  }
                }, 500);
