...
function WeightChart({data, C, F}) {
  ...
  const W=300, H=80;
  const toX = (_,i) => Math.round((i/(data.length-1))*W);
  const toY = w => Math.round(H-((w-minW)/(maxW-minW))*H);
  const pts = data.map((d,i)=>({x:toX(d,i), y:d.w?toY(d.w):null, ...d}));
  ...
  return (
    <div>
      <svg width="100%" viewBox={`0 0 ${W} ${H+18}`} style={{overflow:'visible'}}>
        ...
        <text x={0}  y={H+15} fontSize={9} fill={C.textMuted} fontFamily={F}>{data[0]?.label}</text>
        <text x={W}  y={H+15} fontSize={9} fill={C.textMuted} fontFamily={F} textAnchor="end">{data[data.length-1]?.label}</text>
      </svg>
      <div style={{display:'flex',justifyContent:'space-between',marginTop:6}}>
        ...
        <div style={{fontSize:12,color:C.textSec}}>Hoy: <strong style={{color:C.text}}>{vals[vals.length-1]} kg</strong></div>
      </div>
    </div>
  );
}
    ...
    <div style={{fontFamily:F,minHeight:'100vh',background:C.bg,paddingBottom:88,transition:'background .3s'}}>
      ...
      {newAchiev&&(
        <div style={{
          ...
          color:'white',padding:'13px 20px',borderRadius:22,
          ...
          <div>
            <div style={{fontSize:10,color:'rgba(255,255,255,0.5)',fontWeight:600,letterSpacing:.5,textTransform:'uppercase'}}>Logro desbloqueado</div>
            <div style={{fontSize:14,fontWeight:700,marginTop:1}}>{newAchiev}</div>
          </div>
        </div>
      )}
