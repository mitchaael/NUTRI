      ...
      <div onClick={e=>e.stopPropagation()} style={{width:'100%',background:C.surface,borderRadius:'26px 26px 0 0',padding:'18px 18px 40px',animation:'slideUp .3s ease',maxHeight:'88vh',overflowY:'auto'}}>
        ...
        {sel&&(
          <div style={{animation:'fadeUp .2s ease'}}>
            ...
            <button className="tap" onClick={()=>onAdd({...sel,mins,burn,uid:Date.now()})} style={{width:'100%',padding:'15px',borderRadius:18,border:'none',background:'#34C759',color:'white',fontSize:15,fontWeight:700,fontFamily:F,cursor:'pointer'}}>
...
/* ═══════════════════════════════════════════════════════
   BARCODE SCANNER — cámara + Open Food Facts API
═══════════════════════════════════════════════════════ */
function BarcodeScanner({C, F, onFound, onClose}) {
  const videoRef  = useRef(null);
  const streamRef = useRef(null);
  const rafRef    = useRef(null);
  const [status, setStatus] = useState('init'); // init|scanning|loading|found|error
    ...
    (async()=>{
      try {
        ...
        if(!('BarcodeDetector' in window)){
          setStatus('error'); setErrMsg('Tu navegador no soporta escáner nativo. Usa Chrome en Android.');
          return;
        }
        const det = new window.BarcodeDetector({formats:['ean_13','ean_8','upc_a','upc_e','code_128','code_39']});
        setStatus('scanning');
        const scan = async()=>{
          if(!videoRef.current||videoRef.current.readyState<3){rafRef.current=requestAnimationFrame(scan);return;}
          try{
            const codes = await det.detect(videoRef.current);
            if(codes.length>0){ stopCam(); await lookup(codes[0].rawValue); }
            else rafRef.current = requestAnimationFrame(scan);
          }catch{ rafRef.current=requestAnimationFrame(scan); }
        };
        rafRef.current = requestAnimationFrame(scan);
      } catch(e){
        setStatus('error'); setErrMsg('No se pudo acceder a la cámara. Verifica los permisos.');
      }
    })();
    return stopCam;
  },[]);
    ...
    try{
      ...
      if(data.status===1){
        ...
        const food={
          ...
          marca:p.brands||'Desconocido',
          cat:'Escaneado',
          porcion:parseInt(p.serving_size)||100,
        ...
        setFound(food); setStatus('found');
      } else {
        setStatus('error'); setErrMsg(`Código ${barcode} no encontrado en la base de datos.`);
      }
    }catch{
      setStatus('error'); setErrMsg('Error al buscar el producto. Verifica tu conexión.');
    }
  };

  const retry=()=>{ setStatus('init'); setErrMsg(''); setFound(null); };

  return(
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',zIndex:70,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-end'}}>
      <div onClick={e=>e.stopPropagation()} style={{width:'100%',background:C.surface,borderRadius:'26px 26px 0 0',padding:'18px 18px 44px',animation:'slideUp .3s ease'}}>
        <div style={{width:40,height:4,borderRadius:2,background:C.border,margin:'0 auto 16px'}}/>
        <div style={{fontSize:17,fontWeight:700,color:C.text,marginBottom:16}}>📷 Escanear código de barras</div>

        {/* Camera view */}
        {(status==='scanning'||status==='init')&&(
          <div style={{position:'relative',borderRadius:18,overflow:'hidden',background:'#000',marginBottom:16,aspectRatio:'4/3'}}>
            <video ref={videoRef} autoPlay playsInline muted style={{width:'100%',height:'100%',objectFit:'cover'}}/>
            {/* Aim guide */}
            <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',pointerEvents:'none'}}>
              <div style={{width:'70%',height:100,border:'2px solid rgba(255,255,255,0.8)',borderRadius:12,boxShadow:'0 0 0 2000px rgba(0,0,0,0.35)'}}/>
            </div>
            <div style={{position:'absolute',bottom:12,left:0,right:0,textAlign:'center',fontSize:12,color:'rgba(255,255,255,0.7)',fontFamily:F}}>
        ...
        {status==='loading'&&(
          <div style={{textAlign:'center',padding:'40px 0',marginBottom:16}}>
            ...
            <div style={{fontSize:14,color:C.textSec,fontFamily:F,fontWeight:500}}>Buscando producto...</div>
          </div>
        )}

        {status==='found'&&found&&(
          <div style={{marginBottom:16,animation:'fadeUp .3s ease'}}>
            <div style={{display:'flex',gap:12,alignItems:'center',padding:'14px',background:C.surfaceAlt,borderRadius:16,marginBottom:14,border:`1px solid ${C.border}`}}>
              <span style={{fontSize:36}}>{found.emoji}</span>
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:12,fontWeight:800,color:'#007AFF',marginTop:4}}>{found.cal} kcal / {found.porcion}g</div>
              </div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:7,marginBottom:14}}>
              {[{l:'Prot',v:found.prot,c:C.red},{l:'Carbs',v:found.carbs,c:C.amber},{l:'Grasas',v:found.grasas,c:C.purple},{l:'Fibra',v:found.fibra,c:C.green}].map(({l,v,c2})=>(
                <div key={l} style={{background:C.surfaceAlt,borderRadius:12,padding:'10px 6px',textAlign:'center',border:`1px solid ${C.border}`}}>
                  <div style={{fontSize:16,fontWeight:800,color:C.text}}>{v}g</div>
                  <div style={{fontSize:9,color:C.textMuted,fontWeight:700,textTransform:'uppercase',marginTop:2}}>{l}</div>
                </div>
              ))}
            </div>
            <div style={{display:'flex',gap:8}}>
              ...
              <button className="tap" onClick={()=>onFound(found)} style={{flex:2,padding:'13px',borderRadius:16,border:'none',background:'#007AFF',color:'white',fontSize:14,fontWeight:700,cursor:'pointer',fontFamily:F}}>Agregar a mi diario</button>
            </div>
          </div>
        )}
        ...
        {status==='error'&&(
          <div style={{textAlign:'center',padding:'20px 0',marginBottom:16}}>
            ...
            <div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:6}}>No encontrado</div>
            <div style={{fontSize:12,color:C.textSec,lineHeight:1.5,marginBottom:16}}>{errMsg}</div>
            <button className="tap" onClick={retry} style={{padding:'11px 24px',borderRadius:16,border:`1px solid ${C.border}`,background:C.surfaceAlt,color:C.text,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:F}}>Intentar de nuevo</button>
          </div>
        )}
      </div>
    </div>
        ...
        <div style={{display:'flex',gap:0,background:C.surfaceAlt,borderRadius:14,padding:3,marginBottom:16}}>
          {[{v:'porcion',l:'Por porción'},{v:'gramos',l:'Por gramos'}].map(({v,l})=>(
            <button key={v} onClick={()=>{setMode(v);if(v==='porcion')setGrams(food.porcion||100);}} style={{
              ...
              background:mode===v?(dark?'#3A3A3C':'#FFFFFF'):'transparent',
...
export default function App() {
  ...
  const goTab = (i) => { setPrevTab(tab); setTab(i); };
  ...
  useEffect(()=>{
    ...
    s.textContent=`
      *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
      ...
      .anim-right{animation:slideInRight .28s cubic-bezier(.25,.46,.45,.94) both}
      .anim-left{animation:slideInLeft .28s cubic-bezier(.25,.46,.45,.94) both}
      ...
      .tap:active{opacity:.6;transform:scale(.94)}
  ...
  return (
    <div style={{fontFamily:F,minHeight:'100vh',background:C.bg,paddingBottom:88,transition:'background .3s'}}>
      ...
      {/* ══ BARCODE SCANNER ══ */}
      {showScanner&&<BarcodeScanner C={C} F={F} onClose={()=>setShowScanner(false)} onFound={(food)=>{setShowScanner(false);setDetail(food);}}/>}
              ...
              <div style={{flexShrink:0,position:'relative',width:120,height:120}}>
                <svg width={120} height={120} viewBox="0 0 120 120">
                  ...
                  <circle cx={60} cy={60} r={ringR} fill="none"
                    ...
                    style={{transition:'stroke-dasharray .9s cubic-bezier(.25,.46,.45,.94)'}}/>
                </svg>
