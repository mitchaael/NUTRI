...
function Logo({size=40}) {
  return (
    <svg width={size} height={size} viewBox="0 0 512 512" fill="none">
      <defs>
        <radialGradient id={"lg_bg"+size} cx="35%" cy="30%" r="75%">
          <stop offset="0%" stopColor="#FEFAF0"/>
          <stop offset="100%" stopColor="#E8D5A8"/>
        </radialGradient>
        <radialGradient id={"lg_fr"+size} cx="30%" cy="25%" r="70%">
          <stop offset="0%" stopColor="#FBEC5D"/>
          <stop offset="100%" stopColor="#C8950A"/>
        </radialGradient>
      </defs>
      {/* Background */}
      <rect width="512" height="512" rx="110" fill={"url(#lg_bg"+size+")"}/>
      {/* Red C arc */}
      <path d="M400 256 A156 156 0 1 0 256 412" stroke="#D22B2B" strokeWidth="72" strokeLinecap="round" fill="none"/>
      {/* Leaf */}
      <ellipse cx="210" cy="256" rx="48" ry="58" fill="#2EAA5B" transform="rotate(-18 210 256)"/>
      <ellipse cx="210" cy="256" rx="48" ry="58" fill="none" stroke="#1E7A40" strokeWidth="2" transform="rotate(-18 210 256)"/>
      <line x1="210" y1="204" x2="210" y2="308" stroke="#1A6B38" strokeWidth="3.5" strokeLinecap="round"/>
      <line x1="210" y1="244" x2="190" y2="228" stroke="#1A6B38" strokeWidth="2.5" strokeLinecap="round"/>
      <line x1="210" y1="260" x2="191" y2="248" stroke="#1A6B38" strokeWidth="2.5" strokeLinecap="round"/>
      <line x1="210" y1="276" x2="193" y2="267" stroke="#1A6B38" strokeWidth="2.5" strokeLinecap="round"/>
      <path d="M210 205 Q207 190 202 180" stroke="#7A5C10" strokeWidth="4.5" strokeLinecap="round" fill="none"/>
      {/* Yellow fruit */}
      <circle cx="336" cy="258" r="40" fill={"url(#lg_fr"+size+")"}/>
      <ellipse cx="340" cy="294" rx="24" ry="7" fill="rgba(0,0,0,0.09)"/>
    </svg>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        filter:phase>=1?'drop-shadow(0 8px 32px rgba(0,0,0,0.5))':'none',
      }}>
        <Logo size={96}/>
      </div>
...
Si el usuario pregunta algo fuera de nutrición, redirige amablemente.`;
  ...
  return(
    <div style={{position:'fixed',inset:0,background:C.bg,zIndex:88,display:'flex',flexDirection:'column'}}>
      {/* Header */}
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:'14px 18px',display:'flex',alignItems:'center',gap:12,flexShrink:0}}>
        <button onClick={onClose} style={{background:'none',border:'none',fontSize:16,cursor:'pointer',color:'#007AFF',fontFamily:F,fontWeight:600}}>← Volver</button>
  ...
  return(
    <div style={{position:'fixed',inset:0,background:C.bg,zIndex:85,display:'flex',flexDirection:'column',animation:'fadeUp .3s ease'}}>
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:'14px 18px',display:'flex',alignItems:'center',gap:12,flexShrink:0}}>
        ...
        {active&&<button onClick={reset} style={{background:'none',border:'none',fontSize:11,color:'#FF3B30',cursor:'pointer',fontFamily:F,fontWeight:600}}>Reiniciar</button>}
      </div>
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:50,display:'flex',alignItems:'flex-end'}}>
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        {/* Allergen warning */}
        {getFoodAllergens(food.id).length>0&&(
          <div style={{background:'#FF950018',borderRadius:14,padding:'10px 14px',marginBottom:14,border:'1px solid #FF950040',display:'flex',alignItems:'flex-start',gap:10}}>
            <span style={{fontSize:18,flexShrink:0}}>⚠️</span>
            <div>
              <div style={{fontSize:12,fontWeight:700,color:'#FF9500',marginBottom:4}}>Contiene alérgenos</div>
              <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                {getFoodAllergens(food.id).map(al=>{
                  const info=ALLERGENS.find(x=>x.k===al);
                  if(!info) return null;
                  return <span key={al} style={{fontSize:10,fontWeight:700,color:info.color,background:info.color+'18',padding:'2px 8px',borderRadius:8}}>{info.icon} {info.label}</span>;
                })}
              </div>
            </div>
          </div>
...
@keyframes fadeUp{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}}
  ...
  return (
    <div style={{fontFamily:F,background:C.bg,minHeight:'100svh',transition:'background .3s'}}>
      ...
      <div ref={mainRef} style={{padding:'14px 14px 110px'}}>
