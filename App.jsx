...
function NutriLogo({size=36}) {
  return (
    <svg width={size} height={size} viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect width="44" height="44" rx="12" fill="#285C3E"/>
      {/* Leaf halo */}
      <ellipse cx="21" cy="22" rx="9" ry="13" fill="#6ECC8A" fillOpacity="0.18" transform="rotate(-6 21 22)"/>
      {/* Stylized N — two verticals + diagonal */}
      <rect x="11" y="13" width="4" height="18" rx="2" fill="white"/>
      <rect x="29" y="13" width="4" height="18" rx="2" fill="white"/>
      <path d="M13 13 L33 31" stroke="white" strokeWidth="4" strokeLinecap="round"/>
      {/* Orange accent dot */}
      <circle cx="34" cy="10" r="5" fill="#E07B4A"/>
    </svg>
  );
}
...
/* ═══════════════════════════════════════════
   STEPPER BUTTON
═══════════════════════════════════════════ */
function Stepper({value, onDec, onInc, min=0, max=999}) {
  return (
    <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
      <button onClick={onDec} disabled={value<=min} style={{
        width:30,height:30,borderRadius:9,border:`1.5px solid ${C.border}`,
        background:C.surface,color:value<=min?C.textMuted:C.text,
        fontSize:17,cursor:value<=min?"default":"pointer",fontFamily:F,
      ...
      <button onClick={onInc} disabled={value>=max} style={{
        ...
        fontSize:17,cursor:value>=max?"default":"pointer",fontFamily:F,
        display:"flex",alignItems:"center",justifyContent:"center",transition:"all .15s",
      }}>+</button>
    </div>
  );
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    s.textContent=`
      *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
      ...
      body{margin:0;background:#F2EAE0;}
      @keyframes fadeUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
      .tab-anim{animation:fadeUp .25s ease}
      .food-card:active{transform:scale(.97);transition:transform .1s}
  ...
  const motivacion=(p)=>{
    ...
    if(p<.3)  return "¡Buen comienzo, sigue así! 💚";
    if(p<.6)  return "¡Vas muy bien, a mitad camino! 🌿";
    if(p<.9)  return "¡Excelente progreso, casi en tu meta! 🎯";
    if(p<=1)  return "¡Último empujón, casi llegaste! 🔥";
    return "¡Meta cumplida, así se hace! 🏆";
  };
  ...
  /* ─── RENDER ─── */
  return (
    <div style={{fontFamily:F,minHeight:"100vh",background:C.bg,paddingBottom:88}}>
      ...
      <div style={{
        ...
        boxShadow:"0 2px 18px rgba(27,63,42,0.22)",
      }}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <NutriLogo size={36}/>
            <div>
              <div style={{color:"#fff",fontSize:17,fontWeight:800,letterSpacing:"-.5px",lineHeight:1}}>NutriChile</div>
          ...
          <div style={{textAlign:"right"}}>
            <div style={{color:"#fff",fontSize:14,fontWeight:800,lineHeight:1}}>
              {Math.round(tot.cal)}<span style={{fontSize:11,fontWeight:500,opacity:.55}}> / {metas.cal} kcal</span>
            </div>
            <div style={{color:"rgba(255,255,255,0.45)",fontSize:10,fontWeight:500,marginTop:2}}>
              {tot.cal>metas.cal ? `+${sobre} kcal excedidas` : `${reste} kcal libres`}
            </div>
          </div>
        </div>
        {/* Progress bar */}
        <div style={{height:5,background:"rgba(255,255,255,0.1)",borderRadius:10,overflow:"hidden"}}>
          <div style={{
            ...
            background:`linear-gradient(90deg,${C.primaryGlow},${barColor})`,
            borderRadius:10,
      ...
      <div style={{padding:"14px 14px 0"}}>
        ...
        {tab===0&&<div className="tab-anim">
          ...
          <div style={{marginBottom:14}}>
            <div style={{fontSize:22,fontWeight:800,color:C.text,lineHeight:1.2,letterSpacing:"-.5px"}}>{SALUDO} 👋</div>
            <div style={{fontSize:13,color:C.textSec,fontWeight:500,marginTop:3}}>{motivacion(pct)}</div>
          </div>

          {/* ── CALORIE HERO ── */}
          <div style={{
            background:`linear-gradient(145deg,${C.primary} 0%,${C.primaryDark} 100%)`,
            borderRadius:26,padding:"20px 18px",marginBottom:12,
            boxShadow:"0 10px 32px rgba(27,63,42,0.28)",
            position:"relative",overflow:"hidden",
          }}>
            <div style={{position:"absolute",right:-24,top:-24,width:120,height:120,background:"rgba(255,255,255,0.04)",borderRadius:"50%"}}/>
            <div style={{position:"absolute",left:-30,bottom:-30,width:100,height:100,background:"rgba(255,255,255,0.03)",borderRadius:"50%"}}/>
            <div style={{display:"flex",alignItems:"center",gap:16,position:"relative"}}>

              {/* Ring */}
              <div style={{flexShrink:0,position:"relative",width:130,height:130}}>
                <svg width={130} height={130} viewBox="0 0 120 120">
                  <circle cx={60} cy={60} r={ringR} fill="none" stroke="rgba(255,255,255,0.09)" strokeWidth={10}/>
                  <circle cx={60} cy={60} r={ringR} fill="none"
                    stroke={tot.cal>metas.cal?"#F28B76":"#6ECC8A"} strokeWidth={10}
                ...
                <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
                  <div style={{fontSize:9,color:"rgba(255,255,255,0.45)",fontWeight:600,textTransform:"uppercase",letterSpacing:.8}}>
                    {tot.cal>metas.cal?"Pasaste":"Libres"}
                  </div>
                  <div style={{fontSize:27,fontWeight:800,color:"#fff",lineHeight:1.1,marginTop:2}}>
              ...
              <div style={{flex:1}}>
                <div style={{marginBottom:12}}>
                  ...
                  <div style={{fontSize:30,fontWeight:800,color:"#fff",lineHeight:1}}>
                    {Math.round(tot.cal)}<span style={{fontSize:13,fontWeight:400,opacity:.45}}> kcal</span>
                  </div>
                </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:12}}>
            {[
              {k:"prot",  lS:"Prot",  grad:["#C94B35","#E06555"], shadow:"#C94B3560"},
              {k:"carbs", lS:"Carbs", grad:["#C07C24","#D99240"], shadow:"#C07C2460"},
              {k:"grasas",lS:"Grasas",grad:["#9B4F72","#B5668A"], shadow:"#9B4F7260"},
              {k:"fibra", lS:"Fibra", grad:["#3D8A59","#52A872"], shadow:"#3D8A5960"},
            ].map(({k,lS,grad,shadow})=>{
              const v=tot[k], m=k==="fibra"?25:metas[k];
              const p2=m>0?Math.min(v/m*100,100):0;
                  ...
                  <div style={{position:"relative",width:48,height:48,margin:"0 auto 6px"}}>
                    <svg width={48} height={48} viewBox="0 0 46 46">
                      ...
                      <circle cx={23} cy={23} r={r} fill="none"
                        ...
                        strokeDasharray={`${p2/100*circ} ${circ}`}
                        strokeLinecap="round"
          ...
          <div style={{
            ...
            boxShadow:"0 6px 20px rgba(40,120,168,0.35)",
            position:"relative",overflow:"hidden",
          }}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,position:"relative"}}>
              <div>
                ...
                <div style={{fontSize:11,color:"rgba(255,255,255,0.6)",fontWeight:500,marginTop:1}}>
                  {agua===8?"¡Meta de agua cumplida! 🎉":`${agua} de 8 vasos`}
                </div>
              </div>
              <div style={{
            ...
            <div style={{display:"flex",gap:5,position:"relative"}}>
              {Array.from({length:8}).map((_,i)=>(
                <button key={i} className="tap" onClick={()=>setAgua(i<agua?i:i+1)} style={{
                  ...
                  <span style={{fontSize:16,opacity:i<agua?1:0.3,transition:"opacity .2s"}}>{i<agua?"💧":"○"}</span>
                  <span style={{fontSize:7,fontWeight:700,color:i<agua?"white":"rgba(255,255,255,0.35)"}}>{i+1}</span>
                </button>
              ))}
            </div>
          </div>
              ...
              {MEALS.map(m=>{
                ...
                return (
                  <div key={m} style={{
                    ...
                    borderRadius:20,
                    padding:"12px 14px",
                    boxShadow:hasFood?`0 4px 16px ${MC[m]}22`:"0 2px 10px rgba(27,63,42,0.06)",
                    border:`1.5px solid ${hasFood?MC[m]+"44":C.border}`,
                    ...
                    <div style={{
                      ...
                      background:hasFood?`${MC[m]}18`:C.bg,
                      display:"flex",alignItems:"center",justifyContent:"center",
                      fontSize:20,
                      boxShadow:hasFood?`0 2px 10px ${MC[m]}30`:"none",
                      transition:"all .3s ease",
                    }}>{MI[m]}</div>

                    {/* Content */}
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:hasFood?5:0}}>
                        <span style={{fontSize:13,fontWeight:700,color:C.text}}>{m}</span>
                        {hasFood&&(
                          <span style={{
                            fontSize:11,fontWeight:800,color:MC[m],
                            background:`${MC[m]}18`,
                            padding:"3px 9px",borderRadius:10,
                          }}>{Math.round(cal)} kcal</span>
                        )}
                      </div>
                      {hasFood?(
                        <>
                          <div style={{fontSize:10,color:C.textSec,fontWeight:500,marginBottom:6}}>
                            {items.length} {items.length===1?"alimento":"alimentos"} registrados
                          </div>
                          <div style={{height:4,background:C.bg,borderRadius:4,overflow:"hidden"}}>
                            <div style={{height:"100%",width:`${p2}%`,background:`linear-gradient(90deg,${MC[m]}cc,${MC[m]})`,borderRadius:4,transition:"width .5s ease"}}/>
                          </div>
                        </>
                      ):(
                        <div style={{fontSize:11,color:C.textMuted,fontWeight:500}}>Vacío — toca + para agregar</div>
                      )}
                    </div>
                    ...
                    {!hasFood?(
                      <button className="tap" onClick={()=>{setMeal(m);setTab(1);}} style={{
                        ...
                        background:C.bg,color:C.textMuted,fontSize:20,
                      ...
                      <div style={{
                        ...
                        background:`${MC[m]}18`,
                        display:"flex",alignItems:"center",justifyContent:"center",
                        fontSize:12,fontWeight:800,color:MC[m],flexShrink:0,
                      }}>{items.length}</div>
                    )}
                  </div>
                );
            ...
            {log.length===0&&(
              <div style={{textAlign:"center",padding:"12px 0 4px"}}>
                ...
                <button className="tap" onClick={()=>setTab(1)} style={{
                  ...
                  borderRadius:14,padding:"9px 22px",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:F,
                  boxShadow:"0 4px 14px rgba(40,92,62,0.3)",
        ...
        {tab===1&&<div className="tab-anim">
          ...
          <div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:4,marginBottom:14,scrollbarWidth:"none"}}>
            {MEALS.map(m=>(
              <button key={m} className="tap" onClick={()=>setMeal(m)} style={{
                ...
                boxShadow:meal===m?`0 4px 14px ${MC[m]}50`:"0 1px 6px rgba(27,63,42,0.08)",
          ...
          <div style={{position:"relative",marginBottom:10}}>
            ...
            <input value={q} onChange={e=>setQ(e.target.value)}
              ...
              style={{
                ...
                boxShadow:q?"0 0 0 3px rgba(40,92,62,0.08)":"none",
                transition:"all .2s ease",fontWeight:500,
              }}/>
            {q&&<button onClick={()=>setQ("")} style={{
              position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",
              background:"none",border:"none",fontSize:20,cursor:"pointer",color:C.textMuted,lineHeight:1,
            }}>×</button>}
          </div>
          ...
          <div style={{fontSize:11,color:C.textMuted,fontWeight:600,marginBottom:10}}>{foods.length} productos encontrados</div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {foods.length===0&&(
              <div style={{textAlign:"center",padding:"50px 0"}}>
                ...
                <div style={{fontSize:12,color:C.textMuted,marginTop:4}}>Prueba con otro término</div>
              </div>
            )}
            {foods.map(a=>{
              ...
              return (
                <div key={a.id} className="food-card" onClick={()=>add(a)} style={{
                  ...
                  boxShadow:inLog?`0 4px 16px ${MC[meal]}28`:"0 2px 10px rgba(27,63,42,0.07)",
                  transition:"all .2s ease",cursor:"pointer",
                }}>
                  <div style={{
                    width:46,height:46,borderRadius:14,
                    background:inLog?`${MC[meal]}18`:C.bg,
                    display:"flex",alignItems:"center",justifyContent:"center",
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",gap:6,marginTop:5,flexWrap:"wrap"}}>
                      ...
                      {MACROS.map(({k,lS,c})=>(
                        <span key={k} style={{fontSize:10,fontWeight:700,color:c,background:`${c}18`,padding:"1px 6px",borderRadius:6}}>
                          {lS[0]}:{a[k]}g
                        </span>
                      ))}
                    </div>
                  ...
                  <div style={{
                    ...
                    background:inLog?MC[meal]:C.primary,color:"white",
                    fontSize:20,fontWeight:900,
                    display:"flex",alignItems:"center",justifyContent:"center",
                    flexShrink:0,boxShadow:inLog?`0 4px 12px ${MC[meal]}60`:"0 4px 12px rgba(40,92,62,0.28)",
                    transition:"all .2s ease",
                  }}>+</div>
                </div>
              );
        ...
        {tab===2&&<div className="tab-anim">
          ...
          <div style={{
            ...
            boxShadow:"0 8px 28px rgba(27,63,42,0.24)",
          }}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:4,textAlign:"center"}}>
              {[
                {l:"Kcal",v:Math.round(tot.cal), m:metas.cal,  c:"#fff"},
                {l:"Prot",v:Math.round(tot.prot), m:metas.prot, c:"#FF9B8A"},
                {l:"Carbs",v:Math.round(tot.carbs),m:metas.carbs,c:"#FFD08A"},
                {l:"Grasas",v:Math.round(tot.grasas),m:metas.grasas,c:"#FFA8CE"},
                {l:"Fibra",v:Math.round(tot.fibra),m:25, c:"#7ADEA0"},
              ].map(({l,v,m,c})=>(
                <div key={l}>
          ...
          {log.length===0?(
            <div style={{textAlign:"center",padding:"60px 20px"}}>
              ...
              <div style={{fontSize:13,color:C.textSec,marginBottom:18,fontWeight:500}}>Ve a "Agregar" para registrar tus comidas</div>
              <button className="tap" onClick={()=>setTab(1)} style={{
                background:C.primary,color:"white",border:"none",
                borderRadius:16,padding:"10px 24px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:F,
              }}>Ir a Agregar</button>
            </div>
          ):<>
            {MEALS.map(m=>{
                ...
                <div key={m} style={{marginBottom:16}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:10,color:C.textSec,fontWeight:600,marginTop:1}}>
                        {Math.round(mc.cal)} kcal · P:{Math.round(mc.prot)}g C:{Math.round(mc.carbs)}g G:{Math.round(mc.grasas)}g
                      </div>
                    </div>
                    <div style={{height:1,flex:.3,background:C.border}}/>
                  ...
                  {items.map(item=>(
                    <div key={item.uid} style={{
                      ...
                      background:C.surface,borderRadius:18,padding:"11px 14px",
                      marginBottom:6,
                      boxShadow:"0 2px 10px rgba(27,63,42,0.07)",
                      borderLeft:`3px solid ${MC[m]}`,
                    }}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{display:"flex",gap:5,marginTop:3}}>
                          ...
                          <span style={{fontSize:10,color:"#9B4F72",fontWeight:600}}>G:{Math.round(item.grasas*item.qty)}g</span>
                        </div>
                      </div>
            ...
            <button className="tap" onClick={()=>setLog([])} style={{
              ...
              background:C.surface,color:"#C94B35",
              fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:F,marginTop:4,
        ...
        {tab===3&&<div className="tab-anim">
          ...
          {/* Profile card */}
          <div style={{background:C.surface,borderRadius:22,padding:"18px",marginBottom:12,boxShadow:"0 2px 12px rgba(27,63,42,0.07)"}}>
            ...
            <div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:5}}>
                {[
                  {v:"1.2",  l:"🛋️",lbl:"Sedentario",    sub:"Sin ejercicio"},
                  {v:"1.375",l:"🚶",lbl:"Ligero",         sub:"1–2 días/semana"},
                  {v:"1.55", l:"🏃",lbl:"Moderado",       sub:"3–5 días/semana"},
                  {v:"1.725",l:"💪",lbl:"Activo",         sub:"6–7 días/semana"},
                  {v:"1.9",  l:"🔥",lbl:"Muy activo",     sub:"Trabajo físico / 2x día"},
                ].map(({v,l,lbl,sub})=>(
                  <button key={v} className="tap" onClick={()=>setPerfil({...perfil,act:v})} style={{
                    ...
                    background:perfil.act===v?"#EBF5EE":C.surfaceAlt,
            ...
            <div style={{
              ...
              boxShadow:"0 6px 18px rgba(224,123,74,0.3)",
            }}>
              <div>
                <div style={{fontSize:11,color:"rgba(255,255,255,0.65)",fontWeight:700,textTransform:"uppercase",letterSpacing:.5}}>Tu TDEE</div>
                <div style={{fontSize:11,color:"rgba(255,255,255,0.4)",fontWeight:500,marginTop:2}}>Calorías quemadas al día</div>
              </div>
              <div>
          ...
          <div style={{background:C.surface,borderRadius:22,padding:"18px",marginBottom:12,boxShadow:"0 2px 12px rgba(27,63,42,0.07)"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
              {[
                {k:"bajar",  icon:"🔥",lbl:"Bajar de peso",  sub:"Déficit −500 kcal",c:"#C94B35",bg:"#FBE9E6"},
                {k:"mantener",icon:"⚖️",lbl:"Mantener peso", sub:"= Tu TDEE",          c:"#4A86C8",bg:"#E8F0FB"},
                {k:"recomp", icon:"💪",lbl:"Recomposición",  sub:"TDEE + alta prot.",  c:"#7C5CBF",bg:"#F0EBFB"},
                {k:"subir",  icon:"📈",lbl:"Ganar masa",     sub:"Superávit +300 kcal",c:"#3A9E6A",bg:"#E7F5ED"},
              ].map(({k,icon,lbl,sub,c,bg})=>(
                <button key={k} className="tap" onClick={()=>setObj(k)} style={{
                  padding:"14px 10px",borderRadius:18,
                  border:`2px solid ${obj===k?c:C.border}`,
                  background:obj===k?bg:C.surfaceAlt,
                  cursor:"pointer",fontFamily:F,textAlign:"center",
                  boxShadow:obj===k?`0 4px 16px ${c}25`:"none",
                  transition:"all .2s ease",
                }}>
            ...
            <div style={{background:C.bg,borderRadius:16,padding:"14px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
                {[
                  {lbl:"Calorías",v:metas.cal, u:"kcal",bg:C.primary},
                  {lbl:"Proteínas",v:metas.prot,u:"g",  bg:"#C94B35"},
                  {lbl:"Carbos",   v:metas.carbs,u:"g", bg:"#C07C24"},
                  {lbl:"Grasas",   v:metas.grasas,u:"g",bg:"#9B4F72"},
                ].map(({lbl,v,u,bg})=>(
          ...
          {DIETAS.map(d=>(
            <div key={d.id} style={{
              background:C.surface,borderRadius:22,marginBottom:10,
              border:`2px solid ${expanded===d.id?d.color:C.border}`,
              boxShadow:expanded===d.id?`0 6px 24px ${d.color}22`:"0 2px 10px rgba(27,63,42,0.06)",
              ...
              <div style={{padding:"14px 16px",cursor:"pointer"}} onClick={()=>setExpand(expanded===d.id?null:d.id)}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <div style={{width:50,height:50,borderRadius:15,background:`${d.color}16`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{d.icon}</div>
                  <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:14,color:expanded===d.id?d.color:C.textMuted,transition:"transform .2s ease",transform:expanded===d.id?"rotate(180deg)":"rotate(0deg)"}}>▾</div>
                </div>
              </div>
              {expanded===d.id&&(
                <div style={{padding:"0 16px 16px",borderTop:`1.5px solid ${C.border}`}}>
                  ...
                  {d.menu.map((m,i)=>(
                    <div key={i} style={{marginBottom:6,padding:"10px 12px",background:C.bg,borderRadius:13,borderLeft:`3px solid ${d.color}`}}>
      ...
      <div style={{
        ...
        {[
          ...
          <button key={i} className="tap" onClick={()=>setTab(i)} style={{
            ...
            <div style={{
              ...
              background:tab===i?"#EBF5EE":"transparent",
              display:"flex",alignItems:"center",justifyContent:"center",
              fontSize:tab===i?22:19,
              transition:"all .2s ease",
            }}>{icon}</div>
            <div style={{
              fontSize:10,fontWeight:tab===i?800:500,
