  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      try {
        ...
        // Perfil
        if(data.profile?.nombre && !LS.get('onboarded',false)){
          setNombre(data.profile.nombre);
          if(data.profile.perfil) setPerfil(data.profile.perfil);
          if(data.profile.obj) setObj(data.profile.obj);
          LS.set('nombre',data.profile.nombre);
          LS.set('perfil',data.profile.perfil);
          LS.set('obj',data.profile.obj);
        ...
        // Logs diarios (restaura los últimos 60 días)
        if(data.logs?.length){
          data.logs.forEach(row=>{
            const localLog = LS.get('log_'+row.date,[]);
            if(!localLog.length && row.log?.length){ LS.set('log_'+row.date, row.log); }
            if(!LS.get('agua_'+row.date,null) && row.agua>0){ LS.set('agua_'+row.date, row.agua); }
            if(!LS.get('ex_'+row.date,[]).length && row.exercises?.length){ LS.set('ex_'+row.date, row.exercises); }
          });
          // Si hoy vino de Supabase y local está vacío, cargar
...
export default function App() {
  ...
  useEffect(()=>{
    supabase.auth.getSession().then(({ data: { session } }) => {
      // Si no hay sesión guardada, mostrar auth screen
  ...
  const handleAuth = (user) => {
    if (!user) {
      // Continuar sin cuenta
      localStorage.setItem('caloru_skipAuth', '1');
    }
    setShowAuth(false);
  };
